-- Homework 2:
--   Due: Thursday, Sept. 12 by 3:45pm.
--   Total Points: 9

-- 0. Define the function isIn that takes an element of a
-- list, and a list, and returns true if the element is a member of
-- the list or false otherwise.
isIn :: Eq a => a -> [a] -> Bool
isIn = undefined

-- 1. Define the intersper function which takes two lists, and
...
-- 2. Text compression is important when storing large documents on
-- finite storage devices like harddrives.  In this problem we are
...
-- head y, where y is the head l, then op returns the list where the
-- head is the x consed onto y, and the tail is the tail of y,
-- otherwise simply cons the list containing x to l.
